...
> **关于这个 fork**
>
> 本仓库 fork 自 [XueshiQiao/CCSwitcher](https://github.com/XueshiQiao/CCSwitcher)，增加了**开发机支持**：
> 上游只能看运行它的那台 Mac，这个版本还能监控和切换远程开发机上的 Claude Code 账号，并把它们的
> 用量合并到同一套面板里。
>
...
>   菜单栏都会以网络允许的最快速度反映出来，而不是等一个固定的轮询间隔。也可以直接从菜单里切换
>   任意一台机器的账号。
...
>   面板 —— 会话记录本身从不同步。
> - **按模型的额度窗口**。解析用量 API 的 `limits[]` 数组，于是按模型划分的窗口（Fable、Opus）会和
>   5 小时、每周窗口并列显示。
